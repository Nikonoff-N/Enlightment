...
class Lesson(models.Model):
    ...
    teachers_price = models.FloatField(default=0)
    group = models.ForeignKey(Group,on_delete=models.PROTECT,default=getDefaultGroup)
    state = models.CharField(max_length=1, choices=LESSON_STATES,default="A")
    def __str__(self):
        return f"{self.name} - {self.teacher} : {len(self.clients.all())} учеников - {self.date} - {self.price}"

    def bodycount(self)->str:
        return f"{self.clients.count()}/{self.group.clients.count()}"
