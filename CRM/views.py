...
def ExportClients(request):
    ...
    writer.writerow(["index","name","phones"])
    clients = Client.objects.all()
    index = 0
    for c in clients:
        index+=1
        name = c.name
        phones = c.phone_set.all()
        phonestr = ""
        for p in phones:
            phonestr+=str(p.phone)
        writer.writerow([index,name,phonestr])
    return response
...
def CRM_Lessons(request):
    ...
    if 'month' in request.POST:
        try:
            ...
            pass
    data = Lesson.objects.filter(date__month = month,state="S")
...
#========================================================
def okLesson(request, lesson_id):
    lesson = Lesson.objects.get(pk=lesson_id)
    lesson.state = "S"
    for c in lesson.group.clients.all():
        lesson.clients.add(c)
    lesson.save()
    ...
    if previous_url:
        ...
        # If the previous URL is not available, redirect to a default URL
        return HttpResponseRedirect(reverse('calendar'))

def canselLesson(request, lesson_id):
    lesson = Lesson.objects.get(pk=lesson_id)
    lesson.state = "F"
    lesson.clients.clear()
    lesson.save()
    # Get the URL of the previous page
    previous_url = request.META.get('HTTP_REFERER')
    # Redirect to the previous page
...
class SalaryView(LoginRequiredMixin,View):
    ...
    def get(self,request):
        ...
        template = loader.get_template('crm/salary.html')
        context = {
            "teachers":teachers,
        }
        return HttpResponse(template.render(context, request))
...
def calendar(request):
    if 'date' in request.POST:
        today = datetime.fromisoformat(request.POST['date'])
        #today = datetime.strptime(request.POST['date'], '%Y-%m-%d').date()
    else:
        today = datetime.today()
    next_week = today + timedelta(days = 7)
    previos_week = today + timedelta(days = 7)
    all_groups = Group.objects.filter(archive = False)
    weekday = today.weekday()
    week = []
    for d in range(7):
            delta= timedelta(days = weekday-d)
            date = today - delta
            groups = [g for g in all_groups if checkTime(g.schedule,str(d))]
            lessons = []
            for g in groups:

                try:
                    lesson = Lesson.objects.get(date=date,group=g)
    ...
    template = loader.get_template('crm/calendar.html')
    context = {
        'today':today,
        "next_week":next_week.isoformat(),
        "previos_week":previos_week.isoformat(),
        'week':week,
    }
    return HttpResponse(template.render(context, request))